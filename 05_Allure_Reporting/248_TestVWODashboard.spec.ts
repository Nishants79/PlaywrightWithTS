...
test.describe("VWO — session reuse", () => {
  test("go directly to dashboard — no login @P0 @smoke", async ({
    page,
  }, testInfo) => {
    await test.step("Open VWO dashboard using saved session", async () => {
      ...
      console.log(
        "Open VWO dashboard using saved session — storageState applied, no login form hit",
      );
  ...
  test("go directly to settings — no login @P1 @regression", async ({
    page,
  }, testInfo) => {
    await test.step("Open VWO account settings using saved session", async () => {
      ...
      console.log(
        "Open VWO account settings using saved session — still authenticated",
      );
